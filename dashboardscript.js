/**
 * dashboardscript.js — MoodFlow Dashboard (Full)
...
async function exportData(type) {
    ...
    if (type === 'csv') {
        ...
        download('moodflow-export.csv', csv, 'text/csv');
    } else if (type === 'json') {
        download('moodflow-export.json', JSON.stringify({tests:allTests,weeks:allWeeks},null,2), 'application/json');
    } else if (type === 'pdf') {
        downloadClientPDF();
    }
}
...
async function downloadClientPDF() {
    ...
    pdfContent.innerHTML = `
        <div style="text-align:center;border-bottom:3px solid #3b82f6;padding-bottom:16px;margin-bottom:24px;">
            <h1 style="margin:0;color:#1e40af;font-size:24px;">MoodFlow — Analytics Report</h1>
        ...
        <div style="margin-top:16px;padding:12px;background:#eff6ff;border-radius:8px;font-size:11px;color:#555;text-align:center;">
            Generated by MoodFlow · For personal wellness awareness only · Not a clinical diagnosis
        </div>
    `;

    html2pdf().set({
        margin:[10,10,10,10],
        filename:`moodflow-dashboard-${new Date().toISOString().slice(0,10)}.pdf`,
