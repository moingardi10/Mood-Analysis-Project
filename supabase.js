/**
 * supabase.js — Single global Supabase client for MoodLens
...
(function () {
    ...
    if (typeof window.supabase === 'undefined' || typeof window.supabase.createClient !== 'function') {
        console.error('[MoodLens] Supabase SDK not loaded. Check CDN script tag.');
        return;
    }

    if (window.sb && window.sb.auth) {
        console.log('[MoodLens] Supabase client already initialised.');
        return;
    }
    ...
    try {
        ...
        window.sb = client;
        console.log('[MoodLens] ✅ Supabase client ready → window.sb');
    } catch (err) {
        console.error('[MoodLens] Failed to create Supabase client:', err);
    }
})();